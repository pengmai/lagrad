func @matmul(%A : tensor<3x4xf32>, %B : tensor<4x5xf32>, %C : tensor<f32> {linalg.inplaceable = true}) -> tensor<f32> {
  ...
  %final = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> ()>], iterator_types = ["reduction", "reduction"]} ins(%res : tensor<3x5xf32>) outs(%C : tensor<f32>) {
  ^bb0(%arg0: f32, %arg1: f32):
    %0 = arith.addf %arg0, %arg1 : f32
    linalg.yield %0 : f32
  } -> tensor<f32>
  return %final : tensor<f32>
}
...
func @main() {
  ...
  %C = arith.constant dense<0.0> : tensor<f32>
  %f = constant @matmul : (tensor<3x4xf32>, tensor<4x5xf32>, tensor<f32>) -> tensor<f32>

  // %df = standalone.grad %f : (tensor<3x4xf32>, tensor<4x5xf32>) -> tensor<3x5xf32>, (tensor<3x4xf32>, tensor<4x5xf32>) -> tensor<3x4xf32>
  %first = call_indirect %f(%A, %B, %C) : (tensor<3x4xf32>, tensor<4x5xf32>, tensor<f32>) -> tensor<f32>
  %casted = tensor.cast %first : tensor<f32> to tensor<*xf32>
