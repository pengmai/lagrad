...
func @matsum(%arr : memref<4x4xf32>) -> f32 {
  ...
  %res = scf.for %iv = %0 to %4 step %1 iter_args(%sum_iter = %res_0) -> f32 {
    %partial_res = scf.for %jv = %0 to %4 step %1 iter_args(%sum_inner = %sum_iter) -> f32 {
      %t = load %arr[%iv, %jv] : memref<4x4xf32>
      %sum_next = addf %sum_inner, %t : f32
...
func @main() {
  ...
  %A = tensor_to_memref %cA : memref<4x4xf32>
  %dA = alloca() : memref<4x4xf32>
  %0 = constant 0.0 : f32
  linalg.fill(%dA, %0) : memref<4x4xf32>, f32
  ...
  %U = memref_cast %dA : memref<4x4xf32> to memref<*xf32>
