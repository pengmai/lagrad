...
#map1 = affine_map<(d0) -> ()>
func.func @logsumexp(%A : tensor<4xf64>) -> f64 {
  ...
  %max = scf.for %iv = %c0 to %c4 step %c1 iter_args(%max_it = %max_init_val) -> f64 {
    %arg0 = tensor.extract %A[%iv] : tensor<4xf64>
    %p = arith.cmpf "ogt", %arg0, %max_it : f64
    %next = arith.select %p, %arg0, %max_it : f64
    scf.yield %next : f64
  }
