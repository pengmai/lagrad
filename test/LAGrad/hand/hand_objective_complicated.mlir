func private @mto_pose_params(%theta: tensor<26xf64>) -> tensor<25x3xf64> {
...
func private @mangle_axis_to_rotation_matrix(%angle_axis: tensor<3xf64>) -> tensor<3x3xf64> {
  %c0 = arith.constant 0 : index
  %c1 = arith.constant 1 : index
  %c2 = arith.constant 2 : index
  %zero = arith.constant 0.0 : f64
...
func private @meuler_angles_to_rotation_matrix(%xyz: tensor<3xf64>) -> tensor<3x3xf64> attributes {pure = true} {
  %c0 = arith.constant 0 : index
  %c1 = arith.constant 1 : index
  %c2 = arith.constant 2 : index
  %zero = arith.constant 0.0 : f64
...
func private @mget_posed_relatives(%base_relatives: tensor<22x4x4xf64>, %pose_params: tensor<25x3xf64>) -> tensor<22x4x4xf64> {
  %c0 = arith.constant 0 : index
  %c1 = arith.constant 1 : index
  %c3 = arith.constant 3 : index
...
func @mlir_hand_objective_complicated(
  %theta : tensor<26xf64>,
  %us : tensor<2x2xf64>,
  ...
  %err = scf.for %iv = %c0 to %cpts step %c1 iter_args(%e_outer = %err_init) -> tensor<2x3xf64> {
    %err_partial = scf.for %jv = %c0 to %c3 step %c1 iter_args(%e_inner = %e_outer) -> tensor<2x3xf64> {
      ...
      %corrs_32 = tensor.extract %correspondences[%iv] : tensor<2xi32>
      %corrs = arith.index_cast %corrs_32 : i32 to index
      %u_0 = tensor.extract %us[%iv, %c0] : tensor<2x2xf64>
      %u_1 = tensor.extract %us[%iv, %c1] : tensor<2x2xf64>
      %usum = arith.addf %u_0, %u_1 : f64
      %onemu = arith.subf %one, %usum : f64
      %verts_0_32 = tensor.extract %triangles[%corrs, %c0] : tensor<1084x3xi32>
      %verts_1_32 = tensor.extract %triangles[%corrs, %c1] : tensor<1084x3xi32>
      %verts_2_32 = tensor.extract %triangles[%corrs, %c2] : tensor<1084x3xi32>
      ...
      %vp0 = tensor.extract %vertex_positions[%verts_0, %jv] : tensor<544x3xf64>
      %vp1 = tensor.extract %vertex_positions[%verts_1, %jv] : tensor<544x3xf64>
      %vp2 = tensor.extract %vertex_positions[%verts_2, %jv] : tensor<544x3xf64>
      ...
      %0 = arith.subf %arg0, %hand_point_coord4 : f64
...
func @lagrad_hand_objective_complicated(
  %theta : tensor<26xf64>,
  %us : tensor<2x2xf64>,
  %parents: tensor<22xi32>,
  %base_relatives: tensor<22x4x4xf64>,
  ...
  return %res#0, %res#1 : tensor<26xf64>, tensor<2x2xf64>
}
