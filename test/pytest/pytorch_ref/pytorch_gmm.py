...
def logsumexpvec(x):
    ...
    # Alternate implementation without the +/- max
    # return torch.t(torch.logsumexp(torch.t(x), 0))
...
def log_wishart_prior(d, wishart_gamma, wishart_m, sum_qs, Qdiags, icf):
    n = d + wishart_m + 1
    k = icf.shape[0]
    ...
    out = torch.sum(
        ...
        * wishart_gamma
        * wishart_gamma
        * (torch.sum(Qdiags ** 2, dim=1) + torch.sum(icf[:, d:] ** 2, dim=1))
        - wishart_m * sum_qs
    )

    C = n * d * (math.log(wishart_gamma / math.sqrt(2)))
    return out
    # return out - k * (C - log_gamma_distrib(0.5 * n, d))
