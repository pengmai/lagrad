...
# For debugging
# $BIN/standalone-opt "$KERNELS/matvec.mlir" -take-grads -canonicalize $BUFFERIZE -convert-linalg-to-std

$BIN/standalone-opt "$KERNELS/matvec.mlir" -take-grads -canonicalize $BUFFERIZE -convert-linalg-to-std -convert-std-to-llvm | $EXPORT | $COMPILE > $TMP/externalcall.o
