  ...
  matchAndRewrite(scf::ForOp op, OpAdaptor adaptor,
    ...
    for (auto pair : llvm::enumerate(loopNest.inputTensorOperands)) {
      ...
      if (auto spType = spAnalysis.getSparsityType(inputOperand)) {
        errs() << "sparsity type: " << spType.value() << "\n";
...
private:
  ...
  bool isRowInsertion(tensor::InsertSliceOp op) const {
    auto inferredType = tensor::ExtractSliceOp::inferResultType(
                            op.getType(), op.getStaticOffsets(),
  ...
  matchAndRewrite(tensor::InsertSliceOp op, OpAdaptor adaptor,
    ...
    case HotSparsityType::OneHot: {
      if (indices.value().getType().isa<IndexType>()) {
        ...
        int64_t lastOffset = op.getStaticOffset(op.static_offsets().size() - 1);
        if (lastOffset == 0) {
          valueOffsets.push_back(indices.value());
        } else {
          valueOffsets.push_back(rewriter.create<arith::AddIOp>(
              loc, rewriter.create<arith::ConstantIndexOp>(loc, lastOffset),
              indices.value()));
