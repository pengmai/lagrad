...
void DEPRECATEDpopulatePrimalCache(
  ...
  for (auto cpair : llvm::zip(caches, valuesToCache)) {
    ...
    if (auto tensorType =
      ...
      SmallVector<OpFoldResult> mixedOffsets{forOp.getInductionVar()};
      SmallVector<OpFoldResult> mixedSizes{oneAttr};
      SmallVector<OpFoldResult> mixedStrides{oneAttr};
      for (int i = 0; i < tensorType.getRank(); i++) {
        mixedOffsets.push_back(rewriter.getI64IntegerAttr(0));
        mixedSizes.push_back(
            rewriter.getI64IntegerAttr(tensorType.getShape()[i]));
        mixedStrides.push_back(oneAttr);
      }
      auto view = rewriter.create<memref::SubViewOp>(
          valToCache.getLoc(), resultType, ccache, mixedOffsets, mixedSizes,
          mixedStrides);
      auto memref = rewriter.create<bufferization::ToMemrefOp>(
          valToCache.getLoc(),
          MemRefType::get(tensorType.getShape(), tensorType.getElementType()),
          valToCache);
      rewriter.create<linalg::CopyOp>(valToCache.getLoc(), memref.getResult(),
                                      view.getResult());
    } else {
      ...
      [&](OpBuilder &builder, Location loc, Value iv, ValueRange iterArgs) {
        ...
        for (auto cpair : iterArgsToCached) {
          ...
          if (auto tensorType =
            ...
            auto oneAttr = builder.getI16IntegerAttr(1);
            SmallVector<OpFoldResult> mixedOffsets{idx}, mixedSizes{oneAttr},
                mixedStrides{oneAttr};
            for (int i = 0; i < tensorType.getRank(); i++) {
              mixedOffsets.push_back(builder.getI64IntegerAttr(0));
              mixedSizes.push_back(
                  builder.getI64IntegerAttr(tensorType.getShape()[i]));
              mixedStrides.push_back(oneAttr);
            }

            auto view = builder.create<memref::SubViewOp>(
                cpair.second.getLoc(), resultType, cpair.second, mixedOffsets,
            ...
            if (alloc_new) {
              ...
              builder.create<linalg::CopyOp>(
                  cpair.second.getLoc(), view.getResult(), dest.getResult());
