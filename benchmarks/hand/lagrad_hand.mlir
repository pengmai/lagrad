...
func @mlir_hand_objective_complicated(
  ...
  %err = scf.for %iv = %c0 to %cpts step %c1 iter_args(%e_outer = %err_init) -> tensor<?x3xf64> {
    %err_partial = scf.for %jv = %c0 to %c3 step %c1 iter_args(%e_inner = %e_outer) -> tensor<?x3xf64> {
      %arg0 = tensor.extract %points[%iv, %jv] : tensor<?x3xf64>
      %corrs_32 = tensor.extract %correspondences[%iv] : tensor<?xi32>
      %corrs = arith.index_cast %corrs_32 : i32 to index
      %u_0 = tensor.extract %us[%iv, %c0] : tensor<?x2xf64>
      %u_1 = tensor.extract %us[%iv, %c1] : tensor<?x2xf64>
...
func @lagrad_hand_objective_complicated(
  ...
  return %res#0, %res#1 : tensor<{{ntheta}}xf64>, tensor<?x2xf64>
}
