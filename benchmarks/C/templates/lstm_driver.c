...
// extern double
// mlstm_objective(/*main_params=*/double *, double *, int64_t, int64_t,
// int64_t,
//                 /*extra_params=*/double *, double *, int64_t, int64_t,
//                 int64_t,
...
int main() {
  ...
  // Don't mutate the state in the LSTMInput struct!!
  double *state = (double *)malloc(input.state_sz * sizeof(double));
  ...
  printf("Main size: %d, extra size: %d, state size: %d\n", input.main_sz,
         input.extra_sz, input.state_sz);
  ...
  double max = 0.0;
  for (size_t i = 0; i < input.main_sz; i++) {
    if (fabs(dmain_params[i]) > max) {
      max = fabs(dmain_params[i]);
    }
  }
  printf("Max: %.8e", max);
