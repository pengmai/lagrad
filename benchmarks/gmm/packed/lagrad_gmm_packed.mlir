...
module  {
  ...
  func @mlir_gmm_packed(
    ...
    %slse = scf.for %ix = %c0 to %cn step %c1 iter_args(%slse_iv = %zero) -> f64 {
      %main_term = scf.for %ik = %c0 to %ck step %c1 iter_args(%mt_iter = %main_term_space) -> tensor<{{k}}xf64> {
        ...
        %out_1 = linalg.generic
          ...
          ins(%Ltri_slice, %xcentered : tensor<{{d}}x{{d}}xf64, "pltri">, tensor<{{d}}xf64>)
    ...
    %lwishpri = scf.for %ik = %c0 to %ck step %c1 iter_args(%out_iter = %zero) -> (f64) {
      ...
      %Ltri_slice = tensor.extract_slice %Ls[%ik, 0, 0] [1, {{d}}, {{d}}] [1, 1, 1] : tensor<{{k}}x{{d}}x{{d}}xf64, "pltri"> to tensor<{{d}}x{{d}}xf64, "pltri">
      // Inlined msqnorm_2d
      %fro_1 = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d1, d0)>, #map14], iterator_types = ["reduction", "reduction"]} ins(%Ltri_slice : tensor<{{d}}x{{d}}xf64, "pltri">) outs(%zerod_tensor : tensor<f64>) {
      ^bb0(%arg0: f64, %arg1: f64):
        %0 = arith.mulf %arg0, %arg0 : f64
        %1 = arith.addf %0, %arg1 : f64
        linalg.yield %1 : f64
  ...
  func @lagrad_gmm_packed(
    ...
    %arg3: tensor<{{k}}x{{d}}x{{d}}xf64, "pltri">,
