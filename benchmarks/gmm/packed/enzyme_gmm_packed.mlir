// Building upon gmm_buf_optimized.mlir, this file attempts to compress the Ltri
// representation to improve primal performance.
...
module  {
  ...
  func @msqnorm_2d(%ik: index, %x: memref<{{k}}x{{tri_size}}xf64>) -> f64 {
    %zero = arith.constant 0.0 : f64
    %out = memref.alloc() : memref<f64>
    memref.store %zero, %out[] : memref<f64>
    %c0 = arith.constant 0 : index
    %c1 = arith.constant 1 : index
    %cd = arith.constant {{d}} : index
    %ct = arith.constant {{tri_size}} : index
    scf.for %iv = %c0 to %ct step %c1 {
      %0 = memref.load %x[%ik, %iv] : memref<{{k}}x{{tri_size}}xf64>
      %1 = memref.load %out[] : memref<f64>
      %2 = arith.mulf %0, %0 : f64
  ...
  func @mlog_wishart_prior(%wishart_gamma: f64, %wishart_m: i64, %sum_qs: memref<{{k}}xf64>, %Qdiags: memref<{{k}}x{{d}}xf64>, %Ltri: memref<{{k}}x{{tri_size}}xf64>, %out: memref<f64>) {
    %c0 = arith.constant 0 : index
    ...
    %val = scf.for %ik = %c0 to %ck step %step iter_args(%out_iter = %zero) -> (f64) {
      ...
      %frobenius_1 = call @msqnorm_2d(%ik, %Ltri) : (index, memref<{{k}}x{{tri_size}}xf64>) -> f64
  ...
  func @enzyme_gmm_opt_compressed(%alphas: memref<{{k}}xf64>, %means: memref<{{k}}x{{d}}xf64>, %Qs: memref<{{k}}x{{d}}xf64>, %Ls: memref<{{k}}x{{tri_size}}xf64>, %x: memref<{{n}}x{{d}}xf64>, %wishart_gamma: f64, %wishart_m: i64) -> f64 {
    ...
    call @mlog_wishart_prior(%wishart_gamma, %wishart_m, %sum_qs, %Qdiags, %Ls, %lwishpri_out) : (f64, i64, memref<{{k}}xf64>, memref<{{k}}x{{d}}xf64>, memref<{{k}}x{{tri_size}}xf64>, memref<f64>) -> ()
