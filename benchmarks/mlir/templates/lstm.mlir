...
func @mlstm_objective(
  ...
  %res:2 = scf.for %t = %c0 to %ub step %cb iter_args(%total = %zero, %count = %c0) -> (f64, index) {
    ...
    %xp:2 = scf.for %iv = %c0 to %twob_l step %twob iter_args(%input = %x2, %hidden = %hidden_init) -> (tensor<{{b}}xf64>, tensor<{{b}}xf64>) {
      ...
      %p_1 = arith.cmpi eq, %iv, %c0 : index
      %p = arith.andi %p_0, %p_1 : i1
      scf.if %p {
        %U = tensor.cast %ingate : tensor<{{b}}xf64> to tensor<*xf64>
    ...
    scf.yield %total_next, %count_next : f64, index
  }
