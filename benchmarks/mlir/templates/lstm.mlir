...
func @mlogsumexp(%t: tensor<{{b}}xf64>) -> f64 {
  ...
  %lse = linalg.generic
    {
      indexing_maps = [#map0, affine_map<(d0) -> ()>],
...
func @mlstm_objective(
  ...
  %ub = arith.muli %cc_minus_one, %cb : index
  ...
  %res:3 = scf.for %t = %c0 to %ub step %cb iter_args(%total = %zero, %count = %c0, %state_outer = %state_init) -> (f64, index, tensor<{{state_sz}}xf64>) {
    ...
    // %hidden_init = tensor.extract_slice %state[%c0] [{{b}}] [1] : tensor<{{state_sz}}xf64> to tensor<{{b}}xf64>

    // %state_final:2 = scf.for %iv = %c0 to %twob_l step %twob iter_args(%state = %state_outer, %idx = %c0) -> (tensor<{{state_sz}}xf64>, index) {
    //   %first_iter = arith.cmpi eq, %iv, %c0 : index
    //   %input = scf.if %first_iter -> (tensor<{{b}}xf64>) {
    //     scf.yield %x2 : tensor<{{b}}xf64>
    //   } else {
    //     %iv_prev = arith.subi %iv, %twob : index
    //     %xp_it = tensor.extract_slice %state[%iv_prev] [{{b}}] [1] : tensor<{{state_sz}}xf64> to tensor<{{b}}xf64>
    //     scf.yield %xp_it : tensor<{{b}}xf64>
    //   }
    ...
    //     linalg.yield %2 : f64
    //   } -> tensor<{{b}}xf64>
    //   %state_1 = tensor.insert_slice %cell into %state[%iv_b] [{{b}}] [1] : tensor<{{b}}xf64> into tensor<{{state_sz}}xf64>
    ...
    //   // %p_0 = arith.cmpi eq, %t, %c0 : index
    //   // %p_1 = arith.cmpi eq, %iv, %twob : index
    //   // %p = arith.andi %p_0, %p_1 : i1
    //   // scf.if %p {
    //   //   %U = tensor.cast %hidden_next : tensor<{{b}}xf64> to tensor<*xf64>
    //   //   call @print_memref_f64(%U) : (tensor<*xf64>) -> ()
    //   // }
    //   %state_2 = tensor.insert_slice %hidden_next into %state_1[%iv] [{{b}}] [1] : tensor<{{b}}xf64> into tensor<{{state_sz}}xf64>
    //   // end inlined lstm_model
    //   // scf.yield %hidden_next, %cell : tensor<{{b}}xf64>, tensor<{{b}}xf64>
    //   scf.yield %state_2, %iv : tensor<{{state_sz}}xf64>, index
    // }

    // %xp_final = tensor.extract_slice %state_final#0[%state_final#1] [{{b}}] [1] : tensor<{{state_sz}}xf64> to tensor<{{b}}xf64>
    %xp_final = tensor.extract_slice %state_outer[%c0] [{{b}}] [1] : tensor<{{state_sz}}xf64> to tensor<{{b}}xf64>
    %p = arith.cmpi eq, %t, %cb : index
    scf.if %p {
      %U = tensor.cast %xp_final : tensor<{{b}}xf64> to tensor<*xf64>
      call @print_memref_f64(%U) : (tensor<*xf64>) -> ()
    }
    ...
    %ypred = linalg.generic
      ...
      ins(%xp_final, %w2_1, %w2_2 : tensor<{{b}}xf64>, tensor<{{b}}xf64>, tensor<{{b}}xf64>)
      outs(%zero_b : tensor<{{b}}xf64>) {
    ^bb0(%arg0: f64, %arg1: f64, %arg2: f64, %arg3: f64):
      %0 = arith.mulf %arg0, %arg1 : f64
      %1 = arith.addf %0, %arg2 : f64
    ...
    // scf.yield %total_next, %count_next, %state_final : f64, index, tensor<{{state_sz}}xf64>
    scf.yield %total_next, %count_next, %state_outer : f64, index, tensor<{{state_sz}}xf64>
  }
